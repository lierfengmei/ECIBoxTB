               ...
               UINT  writebuffersize)	// size to the writebuffer
{
  ...
  if (m_bThreadAlive)
  {
    do
    {
      SetEvent(m_hShutdownEvent);	//关闭串口，最高优先级
    } while (m_bThreadAlive);
    TRACE("Thread ended\n");
  }
  ...
  if (m_ov.hEvent != NULL)
    ...
    m_ov.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);		//创建hEvent成员
  

  if (m_hWriteEvent != NULL)
    ResetEvent(m_hWriteEvent);
  else
    m_hWriteEvent = CreateEvent(NULL, TRUE, FALSE, NULL);	//建立m_hWriteEvent写串口事件
  
  if (m_hShutdownEvent != NULL)
    ResetEvent(m_hShutdownEvent);
  else
    m_hShutdownEvent = CreateEvent(NULL, TRUE, FALSE, NULL);//建立m_hShutdownEvent关闭串口事件
  ...
  */
  //打开串口
  m_hComm = CreateFile(szPort,						// communication port string (COMX)，串口名称字符串，如“COM1”
...
线程函数。
监视线路的大致流程：
检查串口→进入循环（WaitCommEvent）不阻塞询问 询问事件→如果有事件到来→到相应处理（关闭/读/写）
*/
UINT CSerialPort::CommThread(LPVOID pParam)
{
  ...
  { 
    ...
    */
    ...
    if (!bResult)	//WaitCommEvent（）返回FALSE,表示返回错误
    { 
    ...
    else	///WaitCommEvent()能正确返回
    {
      ...
      // 如果WaitCommEvent()返回正确，确认buffer中存在待读取的数据。
      ...
      // as it returns.  
      // 如果你一次从buffer中读取超过1byte的数据（此程序中不这么做），那么第一个byte的到达会
	  // 导致WaitForMultipleObjects（）函数停止等待。WaitForMultipleObjects（）函数在m_OverlappedStruct.hEvent中
	  // 重置事件句柄到signelead状态。
    ...
    case 0:
      {
        ...
        // the higest priority and be serviced first.	最高优先级
        ///关断事件，关闭串口
...
//异步方式：
/*
ReadFile由于采用了异步方式，所以它只返回数据是否已经开始读入的状态，并不返回实际的读入数据，即nRealRead无效。
实际读入的数据是由GetOverlappedResult函数返回。
在Windows中，只有在串行设备上才支持异步文件读写，并且GetOverlappedResult函数也只支持串行设备或用DeviceloControl
函数打开的文件。

1.3.2 串口通信协议设计
	双机互联程序使用3种帧：控制帧、数据帧、短语帧。
*/
